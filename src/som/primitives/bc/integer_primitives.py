...
def _to_by_do(_ivkbl, execution_ctx):
    block = execution_ctx.pop_1_any()  # it's also None'd in the og

    by_increment = execution_ctx.pop_1_any()

    limit = execution_ctx.pop_1_any()

    block_method = block.get_method()

    self = execution_ctx.get_tos_any()
