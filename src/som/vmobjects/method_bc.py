...
class BcAbstractMethod(AbstractMethod):
    ...
    def __init__(
        ...
        self._stack_cache_states = [0] * num_bytecodes
    ...
    @jit.elidable_promote("all")
    def get_cache_state(self, index):
        # Get the bytecode at the given index
        assert 0 <= index < len(self._bytecodes)
        return self._stack_cache_states[index]
    ...
    def set_stack_cache_state(self, index, value):
        self._stack_cache_states[index] = value
...
class BcMethod(BcAbstractMethod):
    ...
    def invoke_n(self, execution_ctx):
        ...
        return stack_pop_old_arguments_and_push_result(
            execution_ctx, self._number_of_arguments, result
        )
...
class BcMethodNLR(BcMethod):
    ...
    def invoke_n(self, execution_ctx):
        ...
        try:
            ...
            if e.has_reached_target(inner):
                return stack_pop_old_arguments_and_push_result(
                    execution_ctx, self._number_of_arguments, e.get_result()
                )
            raise e
