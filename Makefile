...
som-ast-jit: core-lib/.git
	SOM_INTERP=AST PYTHONPATH=$(PYTHONPATH):$(PYPY_DIR) $(RPYTHON) --gc=minimark --batch -Ojit src/main_rpython.py

som-bc-jit:	core-lib/.git
	SOM_INTERP=BC  PYTHONPATH=$(PYTHONPATH):$(PYPY_DIR) $(RPYTHON) --gc=minimark --batch -Ojit src/main_rpython.py

som-ast-interp: core-lib/.git
	SOM_INTERP=AST PYTHONPATH=$(PYTHONPATH):$(PYPY_DIR) $(RPYTHON) --gc=minimark --batch src/main_rpython.py

som-bc-interp: core-lib/.git
	SOM_INTERP=BC  PYTHONPATH=$(PYTHONPATH):$(PYPY_DIR) $(RPYTHON) --gc=minimark --batch src/main_rpython.py
